var DataUtil = Backbone.Model.extend({
	...
	rollup : function(tile, meta, cols, offsets, dim2rollup, ranges) {
		...
		var rollupCols = Object.keys(ranges);
...
//				}
	...
	json2img : function(tile, numPerPix, imgSize, max, id) {
		...
		for (var offset = 0; offset < tile.length; ++offset) {	
			...
			case 2:
				...
				//dat[imgOffset + 1] = (0x80 | ((0x0000FF00 & val) >> 8 );
...
//				countt++;
		...
		//$.post("save.php", {data: cv.toDataURL("image/png"), name: id, dir: "tile-png-2"});
